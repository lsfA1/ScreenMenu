...
-(void)loadSelects{
    ...
    [self.tableViewArr enumerateObjectsUsingBlock:^(UITableView *tableView, NSUInteger idx, BOOL * _Nonnull stop) {
        [tableView reloadData];
        // 选中TableView某一行
        [tableView selectRowAtIndexPath:[NSIndexPath indexPathForRow:self->selects[idx] inSection:0] animated:NO scrollPosition:UITableViewScrollPositionNone];
        //  加 !idx 是 循环第一次 idx == 0 方法不执行, 所以循环一次 加载一个TableView.
        if((self->selects[idx] != -1 && !tableView.superview) || !idx) {
    ...
    switch (idx) {
        ...
        case 1:
        {
            ...
            else
            {
                ...
                {
                    NSDictionary *dict=self.dataArr[firstSelectRow];
                    NSArray *arr=dict[@"sub"];
                    return [arr count];
                }
            }
        }
        ...
        case 2:
        {
            ...
            else
            {
                ...
                else
                {
                    NSDictionary *dict=self.dataArr[firstSelectRow];
                    NSArray *arr=dict[@"sub"];
                    NSDictionary *subDict=arr[secondSelectRow];
                    NSArray *subArr=subDict[@"sub"];
                    return [subArr count];
                }
            }
        }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    backgroundViews.backgroundColor = [UIColor colorWithRed:243/255.0 green:243/255.0 blue:243/255.0 alpha:1.0];
    [cell setSelectedBackgroundView:backgroundViews];
    cell.textLabel.highlightedTextColor= [UIColor colorWithRed:55/255.0 green:178/255.0 blue:144/255.0 alpha:1.0];
    return cell;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.tableCount == 1)
    {
        ...
        [self dismiss];
        if(_delegate){
            [_delegate dropMenuViewForGrade:self didSelectName:self.dataArr[indexPath.row]];
        }
    }
    else if (self.tableCount == 3)
    {
        ...
        else if (tableView == self.tableViewArr[2])
        {
            ...
            [cityArr addObject:self.dataArr[firstSelectRow][@"name"]];
            [cityArr addObject:self.dataArr[firstSelectRow][@"sub"][secondSelectRow][@"name"]];
            [cityArr addObject:self.dataArr[firstSelectRow][@"sub"][secondSelectRow][@"sub"][indexPath.row]];
            if(_delegate){
                [_delegate dropMenuViewForCity:self didSelectArray:cityArr];
            }
        }
    }
}
